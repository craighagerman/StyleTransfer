...
def main(device, prompt='', content_dir='', style_dir='', ddim_steps=50, strength=0.5, model=None, seed=42):
    ddim_eta = 0.0
    n_iter = 1
    C = 4
    f = 8
    ...
    type(f"type(prompt): {type(prompt)}")
    type(f"type(data): {type(data)}")
...
if __name__ == "__main__":
    ...
    print(f"type(sampler): {type(sampler)}")
